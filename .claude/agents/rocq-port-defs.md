...
- **Rocq-mirroring prose** (per HOUSE_STYLE.md P1.47, R9, E13). Don't write docstrings, block comments, or inline `--` comments that paraphrase the Rocq source: "Corresponds to Rocq's `bi.foo_lemma`", "Lean port of `Definition foo` from `frac.v`", "in Rocq this is …", "follows the Rocq proof", etc. The Rocq↔Lean mapping is recorded by `@[rocq_alias <rocq.name>]` on the decl — *that* is the documentation of correspondence. Docstrings describe what the Lean code does, in Lean terms, for a reader who has never seen the Rocq source. The Stage-2 reviewer will hard-fail any such hit (check E13).
