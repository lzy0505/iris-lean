...
theorem frag_op_validN {n : Nat} {q1 q2 : G} {a b : A} :
    (✓{n} (◯U{q1} a : UFracAuth F G A) • ◯U{q2} b) ↔ Fraction.Proper (q1 + q2) ∧ ✓{n} (a • b) :=
  frag_validN

@[rocq_alias ufrac_auth_frag_op_valid]
theorem frag_op_valid {q1 q2 : G} {a b : A} :
    (✓ (◯U{q1} a : UFracAuth F G A) • ◯U{q2} b) ↔ Fraction.Proper (q1 + q2) ∧ ✓ (a • b) :=
  frag_valid
...
theorem update_surplus {p q : G} {a b : A} (hp : Fraction.Proper (p + q)) (hv : ✓ (a • b)) :
  ...
  have heq' := heq.trans (CMRA.unit_left_id_dist mz)
  match mz, heq' with
  | none, heq' => cases heq'
  | some (qa, av), heq' =>
    have hpair : ((⟨p⟩ : Frac G), a) ≡{n}≡ (qa, av) := heq'
    constructor
    · refine (.of_eq <| LeibnizO.ext <| (Fraction.add_comm p q).trans ?_)
      exact (congrArg (q + ·) (LeibnizO.eqv_inj (dist_fst hpair)))
    · exact CMRA.op_commN.trans (CMRA.op_right_dist b (dist_snd hpair))
...
theorem update_surplus_cancel {p q : G} {a b : A} [Cancelable b] :
  ...
  | some (qa, av), heq =>
    ...
    constructor
    · refine (.of_eq <| LeibnizO.ext <| Fraction.add_left_cancel (a := q) ?_)
      exact (Fraction.add_comm q p ▸ Fraction.add_comm q qa.car ▸
          (LeibnizO.eqv_inj (dist_fst hpair) : p + q = q + qa.car))
    · exact (cancelableN (CMRA.comm.dist.validN.mp hab)
...
@[rocq_alias ufrac_authURF_contractive]
instance UFracAuthURFContractive (G : Type _) [Fraction G] (T : COFE.OFunctorPre)
    [RFunctorContractive T] : URFunctorContractive (UFracAuthURF G T (F:= F)) :=
  inferInstance
...
@[rocq_alias ufrac_authRF_contractive]
instance UFracAuthRFContractive (G : Type _) [Fraction G] (T : COFE.OFunctorPre)
    [RFunctorContractive T] : RFunctorContractive (UFracAuthRF G T (F:= F)) :=
  inferInstance
