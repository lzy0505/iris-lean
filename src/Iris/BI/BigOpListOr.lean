...
theorem or' {Φ Ψ : Nat → A → PROP} {l : List A} :
    bigOrL (fun k x => iprop(Φ k x ∨ Ψ k x)) l ⊣⊢ bigOrL Φ l ∨ bigOrL Ψ l :=
  BigOpL.op_distr Φ Ψ l

theorem or_2 {Φ Ψ : Nat → A → PROP} {l : List A} :
    bigOrL Φ l ∨ bigOrL Ψ l ⊣⊢ bigOrL (fun k x => iprop(Φ k x ∨ Ψ k x)) l :=
  or'.symm
...
theorem take_drop {Φ : Nat → A → PROP} {l : List A} {n : Nat} :
    bigOrL Φ l ⊣⊢
      bigOrL Φ (l.take n) ∨ bigOrL (fun k => Φ (n + k)) (l.drop n) :=
...
theorem fmap {B : Type _} (f : A → B) {Φ : Nat → B → PROP} {l : List A} :
  ...
  | cons x xs ih =>
    ...
    exact or_congr OFE.Equiv.rfl (ih (Φ := fun n => Φ (n + 1)))
...
theorem sep_l {P : PROP} {Φ : Nat → A → PROP} {l : List A} :
  ...
  calc iprop(P ∗ bigOrL Φ l)
      ⊣⊢ iprop(P ∗ (∃ k, ∃ x, iprop(⌜l[k]? = some x⌝ ∧ Φ k x))) := sep_congr OFE.Equiv.rfl exist
    ...
    _ ⊣⊢ ∃ k, ∃ x, iprop(⌜l[k]? = some x⌝ ∧ (P ∗ Φ k x)) := by
        ...
        calc iprop(P ∗ (⌜l[k]? = some x⌝ ∧ Φ k x))
          ...
          _ ⊣⊢ iprop((<affine> ⌜l[k]? = some x⌝ ∗ P) ∗ Φ k x) := sep_congr sep_comm OFE.Equiv.rfl
...
theorem sep_r {Φ : Nat → A → PROP} {P : PROP} {l : List A} :
  ...
  calc iprop(bigOrL Φ l ∗ P)
    ...
    _ ⊣⊢ bigOrL (fun k x => iprop(Φ k x ∗ P)) l := by
        apply congr; intro k x
        exact sep_comm
...
theorem bind {B : Type _} (f : A → List B) {Φ : B → PROP} {l : List A} :
  ...
  | cons x xs ih =>
    ...
    exact OFE.Equiv.trans app (or_congr OFE.Equiv.rfl ih)
  ...
  | cons x xs ih =>
    ...
    | nil =>
      ...
      calc iprop(▷ bigOrL Φ [x])
        ...
        _ ⊣⊢ bigOrL (fun k x => iprop(▷ Φ k x)) [x] := OFE.Equiv.symm singleton
    | cons y ys =>
