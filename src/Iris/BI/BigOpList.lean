...
theorem wand {Φ Ψ : Nat → A → PROP} {l : List A} :
  ...
  calc iprop(bigSepL Φ l ∗ bigSepL (fun k x => iprop(Φ k x -∗ Ψ k x)) l)
      ⊢ bigSepL (fun k x => iprop(Φ k x ∗ (Φ k x -∗ Ψ k x))) l := (BI.equiv_entails.mp sep_2).1
...
theorem pure_1 {φ : Nat → A → Prop} {l : List A} :
  ...
  | cons y ys ih =>
    ...
    calc iprop(⌜φ 0 y⌝ ∗ bigSepL (fun n x => iprop(⌜φ (n + 1) x⌝)) ys)
      ...
      _ ⊢ iprop(⌜φ 0 y ∧ (∀ k x, ys[k]? = some x → φ (k + 1) x)⌝) := (BI.equiv_entails.mp pure_and).1
...
theorem pure' [BIAffine PROP] {φ : Nat → A → Prop} {l : List A} :
  ...
  · -- ⌜∀ k x, l[k]? = some x → φ k x⌝ ⊢ bigSepL (fun k x => ⌜φ k x⌝) l
    ...
    calc (⌜∀ k x, l[k]? = some x → φ k x⌝ : PROP)
        ⊢ iprop(<affine> ⌜∀ k x, l[k]? = some x → φ k x⌝) :=
          (BI.equiv_entails.mp (affine_affinely _)).2
      _ ⊢ (bigSepL (fun k x => iprop(<affine> ⌜φ k x⌝)) l : PROP) := affinely_pure_2
      _ ⊢ bigSepL (fun k x => iprop(⌜φ k x⌝)) l := mono fun k x _ => affinely_elim
...
theorem omap {B : Type _} (f : A → Option B) {Φ : B → PROP} {l : List A} :
  ...
  | cons x xs ih =>
    ...
    · exact OFE.Equiv.trans ih (OFE.Equiv.symm emp_sep)
    · exact sep_congr OFE.Equiv.rfl ih
...
theorem bind {B : Type _} (f : A → List B) {Φ : B → PROP} {l : List A} :
  ...
  | cons x xs ih =>
    ...
    exact OFE.Equiv.trans app (sep_congr OFE.Equiv.rfl ih)
  ...
  | cons z zs ih =>
    ...
    | zero =>
      ...
      · -- Forward: Φ 0 z ∗ bigSepL ... ⊢ Φ 0 z ∗ ∀ y, Φ 0 y -∗ ...
        ...
        intro y
        apply wand_intro
        exact (BI.equiv_entails.mp sep_comm).1
      ...
      · -- Forward direction
        calc iprop(Φ 0 z ∗ bigSepL (fun n => Φ (n + 1)) zs)
            ⊢ iprop(Φ 0 z ∗ (Φ (j + 1) x ∗ ∀ y, Φ (j + 1) y -∗ bigSepL (fun n => Φ (n + 1)) (zs.set j y))) :=
              sep_mono_r (BI.equiv_entails.mp ih').1
...
theorem lookup_absorbing {Φ : Nat → A → PROP} {l : List A} {i : Nat} {x : A}
    ...
    (h : l[i]? = some x) :
    bigSepL Φ l ⊢ Φ i x :=
  (BI.equiv_entails.mp (lookup_acc h)).1.trans sep_elim_l
...
theorem insert_acc {Φ : Nat → A → PROP} {l : List A} {i : Nat} {x : A}
  ...
  (BI.equiv_entails.mp (lookup_acc h)).1
...
theorem elem_of_acc {Φ : A → PROP} {l : List A} {x : A}
  ...
  have hacc := (BI.equiv_entails.mp (lookup_acc (Φ := fun _ => Φ) hlookup)).1
  ...
  | cons z zs ih =>
    ...
    | zero =>
      ...
      calc bigSepL (fun n => Φ (n + 1)) zs
          ⊣⊢ bigSepL (fun n y => if n + 1 = 0 then emp else Φ (n + 1) y) zs := by
            apply proper
            intro k y _
            simp only [Nat.add_one_ne_zero, ↓reduceIte]
            exact OFE.Equiv.rfl
        _ ⊣⊢ emp ∗ bigSepL (fun n y => if n + 1 = 0 then emp else Φ (n + 1) y) zs := emp_sep.symm
    | succ j =>
      simp only [List.getElem?_cons_succ] at h
      simp only [bigSepL, bigOpL]
      have ih' := ih (i := j) (Φ := fun n => Φ (n + 1)) h
      -- ih' : bigSepL (Φ . + 1) zs ⊣⊢ Φ (j+1) x ∗ bigSepL (fun k y => if k = j then emp else Φ (k+1) y) zs
      calc Φ 0 z ∗ bigSepL (fun n => Φ (n + 1)) zs
          ⊣⊢ Φ 0 z ∗ (Φ (j + 1) x ∗ bigSepL (fun k y => if k = j then emp else Φ (k + 1) y) zs) :=
            sep_congr OFE.Equiv.rfl ih'
        _ ⊣⊢ Φ (j + 1) x ∗ (Φ 0 z ∗ bigSepL (fun k y => if k = j then emp else Φ (k + 1) y) zs) := by
            calc Φ 0 z ∗ (Φ (j + 1) x ∗ bigSepL (fun k y => if k = j then emp else Φ (k + 1) y) zs)
                ⊣⊢ (Φ 0 z ∗ Φ (j + 1) x) ∗ bigSepL (fun k y => if k = j then emp else Φ (k + 1) y) zs := sep_assoc.symm
              _ ⊣⊢ (Φ (j + 1) x ∗ Φ 0 z) ∗ bigSepL (fun k y => if k = j then emp else Φ (k + 1) y) zs := sep_congr sep_comm OFE.Equiv.rfl
              _ ⊣⊢ Φ (j + 1) x ∗ (Φ 0 z ∗ bigSepL (fun k y => if k = j then emp else Φ (k + 1) y) zs) := sep_assoc
  ...
  calc bigSepL Φ l
    ...
    _ ⊣⊢ Φ i x ∗ bigSepL (fun k y => iprop(⌜k ≠ i⌝ → Φ k y)) l := by
        ...
        · -- k = i: need emp ⊣⊢ ⌜False⌝ → Φ k y
          ...
          · -- emp ⊢ ⌜False⌝ → Φ k y
            apply imp_intro'
            exact (pure_elim_l (φ := False) (R := Φ k y)
              (fun hf => False.elim hf)).trans Entails.rfl
          · -- ⌜False⌝ → Φ k y ⊢ emp
            exact Affine.affine
        · -- k ≠ i: need Φ k y ⊣⊢ ⌜True⌝ → Φ k y
...
theorem intro {P : PROP} {Φ : Nat → A → PROP} {l : List A} [Intuitionistic P]
  ...
  | cons y ys ih =>
    ...
    have hdup : iprop(□ P) ⊢ iprop(□ P ∗ □ P) := (BI.equiv_entails.mp intuitionistically_sep_idem).2
...
theorem forall' {Φ : Nat → A → PROP} {l : List A} [BIAffine PROP]
  ...
  have hacc : bigSepL Φ l ⊢ Φ k x ∗ (∀ y, Φ k y -∗ bigSepL Φ (l.set k y)) :=
    (BI.equiv_entails.mp (lookup_acc hget)).1
  ...
  calc bigSepL Φ l
    ...
    _ ⊢ iprop(<pers> Φ k x) := (BI.equiv_entails.mp sep_comm).1.trans persistently_absorb_r
...
theorem impl {Φ Ψ : Nat → A → PROP} {l : List A} :
  ...
  have h1 : iprop(□ (∀ k x, ⌜l[k]? = some x⌝ → Φ k x -∗ Ψ k x)) ⊢ bigSepL (fun k x => iprop(Φ k x -∗ Ψ k x)) l := by
    ...
    calc iprop(□ (∀ k x, ⌜l[k]? = some x⌝ → Φ k x -∗ Ψ k x))
      ...
      _ ⊢ Φ k x -∗ Ψ k x := (BI.equiv_entails.mp (true_imp (P := iprop(Φ k x -∗ Ψ k x)))).1
  calc iprop(bigSepL Φ l ∗ □ (∀ k x, ⌜l[k]? = some x⌝ → Φ k x -∗ Ψ k x))
      ⊢ iprop(bigSepL Φ l ∗ bigSepL (fun k x => iprop(Φ k x -∗ Ψ k x)) l) := sep_mono_r h1
    _ ⊢ bigSepL (fun k x => iprop(Φ k x ∗ (Φ k x -∗ Ψ k x))) l := (BI.equiv_entails.mp sep_2).1
    _ ⊢ bigSepL Ψ l := mono (fun _ _ _ => wand_elim_r)
...
theorem lookup_acc_impl {Φ : Nat → A → PROP} {l : List A} {i : Nat} {x : A}
  ...
  have hdel := (BI.equiv_entails.mp (delete (Φ := Φ) h)).1
  calc bigSepL Φ l
    ...
    _ ⊢ Φ i x ∗ ∀ Ψ, □ (∀ k y, iprop(⌜l[k]? = some y⌝ → ⌜k ≠ i⌝ → Φ k y -∗ Ψ k y)) -∗
        ...
        have hdel_psi := (BI.equiv_entails.mp (delete (Φ := Ψ) h)).2
          ...
          _ ⊢ iprop(Ψ i x ∗ bigSepL (fun k y => if k = i then emp else Ψ k y) l) := by
                ...
                calc iprop(bigSepL (fun k y => if k = i then emp else Φ k y) l ∗
                    ...
                    ⊢ iprop(bigSepL (fun k y => if k = i then emp else Φ k y) l ∗
                        ...
                        · -- k ≠ i: goal is □(...) ⊢ Φ k y -∗ Ψ k y
                          ...
                          calc iprop(□ (∀ k y, ⌜l[k]? = some y⌝ → ⌜k ≠ i⌝ → Φ k y -∗ Ψ k y))
                            ...
                            _ ⊢ iprop(⌜k ≠ i⌝ → Φ k y -∗ Ψ k y) := (BI.equiv_entails.mp true_imp).1
                            _ ⊢ iprop(⌜True⌝ → Φ k y -∗ Ψ k y) := imp_mono_l (pure_mono (fun _ => hki))
                            _ ⊢ Φ k y -∗ Ψ k y := (BI.equiv_entails.mp true_imp).1
                  _ ⊢ bigSepL (fun k y => iprop((if k = i then emp else Φ k y) ∗
                          (if k = i then emp else iprop(Φ k y -∗ Ψ k y)))) l :=
                  ...
                  _ ⊢ bigSepL (fun k y => if k = i then emp else Ψ k y) l := by
                        ...
                        · subst hki
                          simp only [↓reduceIte]
                          exact (BI.equiv_entails.mp emp_sep).1
                        · simp only [hki, ↓reduceIte]
                          exact wand_elim_r
...
theorem later [BIAffine PROP] {Φ : Nat → A → PROP} {l : List A} :
  ...
  | cons x xs ih =>
    ...
    calc iprop(▷ (Φ 0 x ∗ bigSepL (fun n => Φ (n + 1)) xs))
        ⊣⊢ iprop(▷ Φ 0 x ∗ ▷ bigSepL (fun n => Φ (n + 1)) xs) :=
          BI.equiv_entails.mpr ⟨later_sep_1, later_sep_2⟩
...
theorem later_2 {Φ : Nat → A → PROP} {l : List A} :
  ...
  | cons x xs ih =>
    ...
    calc iprop(▷ Φ 0 x ∗ bigSepL (fun n y => iprop(▷ Φ (n + 1) y)) xs)
      ...
      _ ⊢ iprop(▷ (Φ 0 x ∗ bigSepL (fun n => Φ (n + 1)) xs)) := later_sep_2
...
theorem submseteq {Φ : A → PROP} [∀ x, Affine (Φ x)] {l₁ l₂ l : List A}
  ...
  have hperm := (BI.equiv_entails.mp (perm (Φ := Φ) h)).2
  have happ := (BI.equiv_entails.mp (app (Φ := fun _ => Φ) (l₁ := l₁) (l₂ := l))).1
...
theorem dup {P : PROP} [Affine P] {l : List A} :
  ...
  | cons x xs ih =>
    ...
    have hdup : iprop(□ (P -∗ P ∗ P)) ⊢ iprop(□ (P -∗ P ∗ P) ∗ □ (P -∗ P ∗ P)) :=
      (BI.equiv_entails.mp intuitionistically_sep_idem).2
    ...
    calc iprop(□ (P -∗ P ∗ P) ∗ P)
      ...
      _ ⊢ P ∗ bigSepL (fun _ _ => P) xs := (BI.equiv_entails.mp sep_comm).1
    ...
    | cons y ys =>
      ...
      calc (Φ 0 x ∗ Ψ 0 y) ∗ bigSepL (fun i xy => iprop(Φ (i + 1) xy.1 ∗ Ψ (i + 1) xy.2)) (xs.zip ys)
        ...
        _ ⊣⊢ Φ 0 x ∗ (Ψ 0 y ∗ (bigSepL (fun n => Φ (n + 1)) xs ∗ bigSepL (fun n => Ψ (n + 1)) ys)) :=
            sep_assoc
        _ ⊣⊢ Φ 0 x ∗ ((Ψ 0 y ∗ bigSepL (fun n => Φ (n + 1)) xs) ∗ bigSepL (fun n => Ψ (n + 1)) ys) :=
            sep_congr OFE.Equiv.rfl sep_assoc.symm
        _ ⊣⊢ Φ 0 x ∗ ((bigSepL (fun n => Φ (n + 1)) xs ∗ Ψ 0 y) ∗ bigSepL (fun n => Ψ (n + 1)) ys) :=
            sep_congr OFE.Equiv.rfl (sep_congr sep_comm OFE.Equiv.rfl)
        _ ⊣⊢ Φ 0 x ∗ (bigSepL (fun n => Φ (n + 1)) xs ∗ (Ψ 0 y ∗ bigSepL (fun n => Ψ (n + 1)) ys)) :=
            sep_congr OFE.Equiv.rfl sep_assoc
        _ ⊣⊢ (Φ 0 x ∗ bigSepL (fun n => Φ (n + 1)) xs) ∗ (Ψ 0 y ∗ bigSepL (fun n => Ψ (n + 1)) ys) :=
            sep_assoc.symm
  ...
  | cons x xs ih =>
    ...
    | nil =>
      ...
      calc (emp : PROP)
        ...
        _ ⊣⊢ emp ∗ bigSepL (fun _ _ => (emp : PROP)) xs := sep_congr OFE.Equiv.rfl emp_l.symm
    | cons y ys =>
